...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(128 * 18 * 18, 512)  # Adjusted for 150x150 input size
    ...
    def forward(self, x):
        ...
        x = x.view(-1, 128 * 18 * 18)  # Adjusted for 150x150 input size
        x = torch.relu(self.fc1(x))
        x = self.dropout(x)
        x = self.fc2(x)
        return x

#__main__ is wrapped in '' to allow multiple processes to be spawned for parallel processing
if __name__ == '__main__':
    ...
    for category in categories:
        ...
        train_folder = os.path.join(data_dir, category, 'augmented')
